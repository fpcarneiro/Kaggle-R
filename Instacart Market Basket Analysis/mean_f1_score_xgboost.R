...
xgb_eval_f1 <- function (yhat, dtrain, nrounds) {
...
xgb_eval_f1 <- function (yhat, dtrain) {
  return (list(metric = "f1", value = 0))
}

print("Load datasets")
ais <- fread("Data/aisles.csv", key = "aisle_id")
dept <- fread("Data/departments.csv", key = "department_id")
prod <- fread("Data/products.csv", key = c("product_id","aisle_id", "department_id"))
opp <- fread("Data/order_products__prior.csv")
opt <- fread("Data/order_products__train.csv")
ord <- fread("Data/orders.csv")
