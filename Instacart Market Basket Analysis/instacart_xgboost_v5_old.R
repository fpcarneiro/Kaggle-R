...
products_stat <- function(orders_set, orders_products_dt, up_dt) {
  ...
  if(missing(orders_set)) {
    ...
    aux <- orders_products[eval_set == "prior", .(mean_basket_size = mean(basket_size, na.rm = T)), by = .(product_id)]
  } else {
    prd <- orders_products_dt[order_id %in% orders_set,
                              .(product_time = .N, prod_reorders = sum(reordered)), by = .(user_id,product_id)]
    num_users <- orders_products[order_id %in% orders_set,length(c(unique(user_id)))]
    aux <- orders_products[order_id %in% orders_set, .(mean_basket_size = mean(basket_size, na.rm = T)), by = .(product_id)]
  }
...
users_stat <- function(orders_set, orders_dt, orders_products_dt, up_dt) {
  ...
  if(missing(orders_set)) {
    ...
    user_mode <- orders_dt[eval_set == 'prior', .(user_mode_basket = Mode(basket_size)), by = .(user_id)]
    ...
    user_mode <- orders_dt[order_id %in% orders_set, .(user_mode_basket = Mode(basket_size)), by = .(user_id)]
...
#                    'up_orders_since_last_order','up_order_rate_since_first_order', 'up_average_dow', 'up_average_hour_of_day', 'up_days_since_prior_order','up_mean_interval')
# 
...
#                      'user_median_interval','user_sd_interval','user_median_basket','user_sd_basket', 'user_mode_basket', 'user_distinct_departments','user_distinct_aisles')
# 
# prd_predictors <- c('prod_n_orders','prod_n_reorders','prod_n_users','prod_n_users_twice','mean_basket_size',
#                     'prod_reorder_probability','prod_reorder_times','prod_reorder_ratio','prod_popularity','prd_mean_interval','prd_median_interval','prd_sd_interval')
# 
...
prd_predictors <- c('prod_n_orders','prod_n_reorders','prod_n_users','prod_n_users_twice','mean_basket_size',
                    'prod_reorder_probability','prod_reorder_times','prod_reorder_ratio','prod_popularity','prd_mean_interval')
...
for(sub in seq(0.3, 1, by = 0.1)) {
  ...
  subtrain <- as.data.frame(train) %>% sample_frac(1.0)
  ...
  watchlist <- list(dtrain=X_all)
  
  for(r in seq(90, 90, by = 1)) {
    set.seed(3000)
    model_all <- xgboost(data = X_all, params = params, nrounds = 100, verbose = 1)
    model_all <- xgb.train(data = X_all, params = params, nrounds = 5, verbose = 1, print_every_n = 1, watchlist = watchlist)
    ...
    train$reordered_all <- predict(model_all, X_all)
    
    for(treshold in seq(0.0, 0.25, by = 0.01)) {
      train$reordered_all_class <- (train$reordered_all > treshold) * 1
      f1 <- f1Score(train$reordered, train$reordered_all_class, cutoff=treshold)
      print( paste("Treshold ", treshold," scored :",  f1 ) )
    }
    
    for(treshold in seq(0.2, 0.21, by = 0.01)) {
      test$reordered_all_class <- (test$reordered_all > 0.18) * 1
